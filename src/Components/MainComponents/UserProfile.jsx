    ...
    <div className="p-4">
      ...
      {user ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div><strong>Role:</strong> {user.role === 1 ? 'Admin' : 'User'}</div>
          <div><strong>Approved:</strong> {user.approved ? 'Yes' : 'No'}</div>
          <div><strong>Blocked:</strong> {user.isBlocked ? 'Yes' : 'No'}</div>
          <div><strong>Department ID:</strong> {user.departmentId}</div>
          <div><strong>Current Payroll:</strong> {user.currentPayroll}</div>
          <div><strong>Promotion Date:</strong> 
            {user.promotionDate ? new Date(user.promotionDate).toLocaleDateString() : 'N/A'}
          </div>
        </div>
      ) : (
        <p>No user data found.</p>
